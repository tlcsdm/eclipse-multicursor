package com.tlcsdm.eclipse.multicursor.handlers;
...
import com.tlcsdm.eclipse.multicursor.common.TextUtil;
...
public class SelectNextOccurrenceHandler extends AbstractHandlerWithState {
	...
	private static final class SelectInProgress {
		...
		public SelectInProgress(Point startingSelection, String selectedText, Set<IRegion> existingSelections,
				int nextOffset) {
	...
	private void startEditing(ISourceViewer viewer) throws ExecutionException {
		...
		try {
			...
			if (LinkedModeModel.getModel(document, 0) != null && currentState != null
					&& selectedRange.equals(currentState.startingSelection)
	...
	// used
	private void startLinkedEdit(Set<IRegion> selections, ITextViewer viewer, Point originalSelection)
			throws BadLocationException {
		...
		for (IRegion selection : selections) {
			linkedPositionGroup.addPosition(
					new LinkedPosition(viewer.getDocument(), selection.getOffset(), selection.getLength()));
		}
